...
function randomIndex() {
    let value = datas[Math.floor(Math.random() * datas.length)];
    if (check_datas.includes(value)) {
        randomIndex();
    } else {
        quoteText.innerText = value.quoteText;
        quoteAuthor.innerText = value.quoteAuthor;
        check_datas.push(value);
    }
}
randomIndex();
reload.addEventListener("click", randomIndex)